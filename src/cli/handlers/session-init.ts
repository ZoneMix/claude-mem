...
export const sessionInitHandler: EventHandler = {
  async execute(input: NormalizedHookInput): Promise<HookResult> {
    ...
    const initResponse = await fetchWithAuth(`http://127.0.0.1:${port}/api/sessions/init`, {
      method: 'POST',
    ...
    if (input.platform !== 'cursor' && sessionDbId) {
      ...
      const response = await fetchWithAuth(`http://127.0.0.1:${port}/sessions/${sessionDbId}/init`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
